...
(function($){
	...
	$.widget( "hosh.plugin_system_listelement_default", $.hosh.plugin_system_form, {
		...
		_create: function(){
			...
			this.element.find('input[name=type]').blur(function(){
				if (texttype != $(this).val()){
					that.element.find('input[name=actionpost_system_listelement]').val('');
					that.submitForm();
					return false;
				}
			}).focus(function(){
		...
		UpdateListElementForm: function(){
			...
			$("#hoshform_system_list").plugin_system_list_default("option","url_task",this.options.url_task);
			$("#hoshform_system_list").plugin_system_list_default("UpdateListElement");
		}
	});
