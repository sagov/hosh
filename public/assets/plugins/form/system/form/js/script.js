...
(function ($) {
    ...
    $.widget("hosh.plugin_system_form_default", $.hosh.plugin_system_form, {
        ...
        _DeleteDisplayGroup: function (group) {
            var that = this;
            that._confirm('Удалить группу ' + that.element.find('#groupname_' + group).text() + ' ?',
                function () {
                    that._setTask({
        ...
        _Clone: function () {
            var that = this;
            that._confirm('Подтвердите действие клонирования',
                function () {
                    that._setTask({
        ...
        _RemoveClone: function () {
            var that = this;
            that._confirm('Подтвердите действие удаления наследования',
                function () {
                    that._setTask({
        ...
        refresh: function () {
            ...
            this.element.find(".displaygroups-element").sortable({
                ...
                stop: function (event, ui) {
                    var that2 = this;
                    that._confirm("Изменить позицию?",
                        function () {
                            that._setChangeOrderDisplayGroup(that2);
                        }, function () {
