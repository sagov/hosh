<?php
require_once 'Hosh/Form/Helper/Abstract.php';

class HoshPluginForm_System_Formdisplaygroup_Helper_Db_GetData extends Hosh_Form_Helper_Abstract
{

    public function render ($options)
    {
        $form = $this->getObject();
        $updateparams = $form->getSetting('updateparams');
        $idform = $updateparams['idowner'];
        $idgroup = $updateparams['id'];
        
        $manager_form = new Hosh_Manager_Form();        
        $form_data = $manager_form->getObject($idform);
        $result_data = $form_data;
        if (isset($form_data['options'])) {
            unset($result_data['options']);
            $options_form = Zend_Json::decode($form_data['options']);
            $result_data = array_merge($result_data, $options_form);
        }
        
        if (! isset($result_data['displaygroup']['items'][$idgroup])) {
            return array();
        }
        $result_data = $result_data['displaygroup']['items'][$idgroup];
        
        $arr_result = $result = array();
        foreach ($result_data as $key_1 => $val_1) {
            if (is_array($val_1)) {
                $this->toArray($key_1, $val_1, $arr_result);
            } else {
                $result[strtolower($key_1)] = $val_1;
            }
        }
        $result = array_merge($result, $arr_result);
        $result['id'] = $idgroup;
        $result['idowner'] = $idform;
        
        return $result;
    }

    protected function toArray ($keyname, $arr, & $result = array())
    {
        if ($keyname) {
            $preff = $keyname . '_';
        } else {
            $preff = null;
        }
        foreach ($arr as $key => $val) {
            if (is_array($val)) {
                $this->toArray(strtolower($preff . $key), $val, $result);
            } else {
                $result[strtolower($preff . $key)] = $val;
            }
        }
    }
}		