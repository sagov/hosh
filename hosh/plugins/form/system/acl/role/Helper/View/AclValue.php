...
class HoshPluginForm_System_Acl_Role_Helper_View_AclValue extends Hosh_Form_Helper_Abstract
{
    public function render($options)
    {
        ...
        foreach ($listacl as $val){
            ...
            $item = null;
            $item  .= '<tr class="item-value item-acl-'.$val['id'].''.$classitem.'" data-bdeny="'.$bdeny.'">';
            $item  .= '<td class="state">';
            $item  .= (isset($icon['state'])) ? $icon['state'] : '&nbsp;';
            $item  .= '</td>';
            $item  .= '<td class="scaption-item"><a href="'.$view->Hosh_Url(array('controller'=>'ref_acl_value','action'=>'view','id'=>$val['id'])).'" target="_blank">'.$translate->_($val['scaption']).' ('.$val['sname'].')</a></span></td>';
            $item  .= '<td>';
