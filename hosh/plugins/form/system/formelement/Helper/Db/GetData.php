...
class HoshPluginForm_System_Formelement_Helper_Db_GetData extends Hosh_Form_Helper_Abstract
{
	public function render($options){
		$form = $this->getObject();
		$updateparams = $form->getSettings('updateparams');
		$id = $updateparams['id'];
		$packageform_element = new Hosh_Manager_Db_Package_Hosh_Form_Element();
		$form_data_element = $packageform_element->getObject($id);
		
		
		if (!empty($form_data_element['idowner'])){
		  $manager_form = new Hosh_Manager_Form();
		  $form_data = $manager_form->getObject($form_data_element['idowner']);
		  $form_data_element['snamekindowner'] = $form_data['snamekind'];
		}
