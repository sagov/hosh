...
class HoshPluginForm_System_List_Helper_Title extends Hosh_Form_Helper_Abstract
{
	...
	$title = $xhtml = null;
		if ($form->isEdit()){
		    $title = 'List # '.$form->getData('id');
			$xhtml =  '<h1><span class="glyphicon glyphicon-th-list"></span>&nbsp; 
			        <a href="javascript:void(0);"  data-toggle="modal" data-target="#FormInfoModal">'.$title.'</a>';			
			$idclone = $form->getData('idclone');
			if (!empty($idclone)){
			    $xhtml .= ' <i class="fa fa-long-arrow-right" aria-hidden="true"></i> <a href="'.$view->Hosh_Url(array('controller'=>'ref_list','action'=>'view','id'=>$idclone)).'"># '.$idclone.'</a>';
			}
			$xhtml .= '</h1>';
		    ...
		    $title = $translate->_('SYS_LIST_NEWLIST');
			$xhtml =  '<h1><span class="glyphicon glyphicon-th-list"></span>&nbsp; '.$title.'</h1>';
		}
		if (!empty($title)){
